...
def install_skill(url: str, home: E.Path) -> E.EffectGen[str, InstallError, Services]:
    ...
    yield from fs.make_directory(skill_dir, recursive=True)
    yield from fs.write_file_string(skill_dir / "SKILL.md", body)
    ...
    if link_exists:
        ...
        yield from fs.make_directory(link.parent, recursive=True)
        yield from fs.symlink(skill_dir, link)
