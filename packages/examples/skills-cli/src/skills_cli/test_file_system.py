...
def test_live_exists_reflects_the_real_file_system(tmp_path):
    ...
    before = E.run_sync_exit(fs.exists(target))
    target.write_text("hi")
    after = E.run_sync_exit(fs.exists(target))
...
def test_live_mkdir_creates_nested_directories_and_is_idempotent(tmp_path):
    ...
    first = E.run_sync_exit(fs.mkdir(nested))
    second = E.run_sync_exit(fs.mkdir(nested))
...
def test_live_write_text_round_trips(tmp_path):
    ...
    result = E.run_sync_exit(fs.write_text(target, "skill body"))
...
def test_live_symlink_points_at_the_target(tmp_path):
    ...
    result = E.run_sync_exit(fs.symlink(link, target))
...
def test_live_effects_are_reusable_values(tmp_path):
    ...
    E.run_sync_exit(write)
    target.unlink()
    rerun = E.run_sync_exit(write)
...
def test_live_exists_counts_a_dangling_symlink_as_existing(tmp_path):
    ...
    result = E.run_sync_exit(fs.exists(link))
...
def test_live_exists_fails_without_search_permission_on_the_parent(tmp_path):
    ...
    result = E.run_sync_exit(fs.exists(probe))
    locked.chmod(0o755)
...
def test_live_mkdir_fails_without_write_permission_on_the_parent(tmp_path):
    ...
    result = E.run_sync_exit(fs.mkdir(target))
...
def test_live_mkdir_fails_when_a_file_occupies_the_path(tmp_path):
    ...
    result = E.run_sync_exit(fs.mkdir(occupied))
...
def test_live_write_text_fails_without_write_permission_on_the_parent(tmp_path):
    ...
    result = E.run_sync_exit(fs.write_text(target, "body"))
    readonly.chmod(0o755)
...
def test_live_symlink_fails_without_write_permission_on_the_parent(tmp_path):
    ...
    result = E.run_sync_exit(fs.symlink(link, tmp_path / "target"))
    readonly.chmod(0o755)
...
def test_test_impl_records_mutations():
    ...
    E.run_sync_exit(fs.mkdir(Path("/skills")))
    E.run_sync_exit(fs.write_text(Path("/skills/SKILL.md"), "body"))
    E.run_sync_exit(fs.symlink(Path("/link"), Path("/skills")))
...
def test_test_impl_exists_consults_files_dirs_and_links():
    ...
    assert E.run_sync_exit(fs.exists(Path("/f"))) == E.Succeeded(value=True)
    assert E.run_sync_exit(fs.exists(Path("/d"))) == E.Succeeded(value=True)
    assert E.run_sync_exit(fs.exists(Path("/l"))) == E.Succeeded(value=True)
    assert E.run_sync_exit(fs.exists(Path("/missing"))) == E.Succeeded(value=False)
