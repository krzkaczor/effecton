...
# The chain starts empty and accumulates provided types as a union.
assert_type(E.RequirementProvider(), E.RequirementProvider[Never])
assert_type(
    E.RequirementProvider().and_provide(Db)(Db("postgres://x")),
    E.RequirementProvider[Db],
)

# Providing the only requirement yields a runnable effect (R = Never).
runnable_one = (
    E.RequirementProvider().and_provide(Db)(Db("postgres://x")).apply(E.require(Db))
)
assert_type(runnable_one, E.Effect[Db])
assert_type(E.run_sync(runnable_one), E.Succeeded[Db] | E.Failure)

# Providing all requirements in one chain discharges R in a single apply.
runnable_two = (
...
# Three requirements — the case where one-at-a-time subtraction used to
# collapse R — discharge fine when provided in one chain.
runnable_three = (
    E.RequirementProvider()
    .and_provide(Db)(Db("postgres://x"))
    .and_provide(Logger)(Logger("info"))
...
# Over-providing works: the requirements only need to be a subset of
# the chain (R is covariant).
over_provided = (
    E.RequirementProvider()
    .and_provide(Db)(Db("postgres://x"))
    .and_provide(Cache)(Cache(1))
...
assert_type(over_provided, E.Effect[Db])

# --- requirements: negative tests ---
