"""Type-level pins for provide. Nothing here runs: ty checks the function
...
def _provide_subtracts_one_requirement_from_r_per_call() -> None:
    ...
    assert_type(
        E.run_sync_exit(runnable_three),
    ...
    # Over-provision is a well-typed no-op: R's covariance means an absent
    # member subtracts nothing, so it cannot be rejected (that would need
    # type negation) and R stays unchanged.
    assert_type(
        runnable_three.provide(Db)(Db("postgres://x")),
...
def _provide_protocol_classes_work_as_requirement_keys() -> None:
    @runtime_checkable
...
def _provide_subclass_implementations() -> None:
    @dataclass(frozen=True)
    class PgDb(Db):
        pass
    ...
    # KNOWN LIMITATION: requirement lookup uses the exact type at runtime,
    # but R's covariance lets a provided supertype cover a subtype
    # requirement statically. The type checker accepts the following
    # program (PgDb is assignable to Db), yet running it dies with
    # MissingRequirement(PgDb): the env holds the Db key and require(PgDb)
    # looks up PgDb. Use exact types as requirement keys.
    assert_type(
