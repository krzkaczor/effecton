...
effecton is a typed effect system for Python inspired by Effect-TS: sync-only, zero runtime dependencies, Python 3.14, typechecked with ty. Kernel modules live in `packages/effecton/src/effecton/`, std services in `packages/effecton/src/effecton/std/`, and tests in `packages/effecton/tests/`.
...
- **Prefer `yield from` over bare `yield`** in `@gen` programs. `Effect.__iter__` is meant to type the sent-back value; ty doesn't thread it through `yield from` yet (it infers `Unknown` for now — canaries in `tests/test_types_gen.py` flag when that changes), but bare `yield` types as `Any` with no path to ever being checked.
