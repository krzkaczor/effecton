...
effecton is a typed effect system for Python inspired by Effect-TS: sync-only, zero required runtime dependencies (aiofiles is an optional extra behind `FileSystem.AsyncLive`), Python 3.14, typechecked with ty. Kernel modules live in `packages/effecton/src/effecton/`, std services in `packages/effecton/src/effecton/std/`, and tests collocated next to the module they cover as `test_<module>.py` (the release workflow deletes every `test_*.py` under `src/` before building the published package).
...
- **Errors are cause-specific, never generic buckets**: one frozen `EffectonError` dataclass per failure cause, such as `FileNotExists` or `HttpStatusError`. Signatures carry precise unions through `type` aliases. `attempt` error mappers translate only the expected exception types and re-raise the rest so they stay defects. Errors are leaves: mark every error dataclass `@final` and never subclass one, so `catch(T)`'s runtime `isinstance` check and the static subtraction of `T` from `E` always agree. Catch by class, never by predicate. `packages/effecton/src/effecton/std/file_system.py` is the exemplar.
- **Errors live in the module that raises them, never in a central `errors.py`**: service errors in the service module, program-level errors alongside the program, each with a module-local union alias (`type FileSystemError = ...`). Cross-module unions compose through the module namespace from the exact errors the called operations raise (`config.ConfigError | E.FileSystem.FileNotFound | E.FileSystem.PermissionDenied`), never through a service-wide umbrella such as `E.FileSystem.FileSystemError`.| E.FileSystem.FileSystemError`). Each error overrides `__str__` with its human-readable message, so rendering a failure is just `str(error)` — no CLI-side describe-match. `packages/changesets` is the exemplar.
...
- **Service pattern**: one module per service, exporting `Protocol`, `Live`, and `Test`. Consumers alias the module: `from ... import git as Git`, then `Git.Protocol`; std services are reached as `E.Clock`, `E.Random`, `E.FileSystem`, `E.Process`.
...
- **Paths are `E.Path` and every read or write goes through `E.FileSystem`**: `E.Path` has no I/O members on purpose, and ruff bans `pathlib`, `os.path`, `shutil`, `tempfile` and the `os` file functions everywhere but the FileSystem and Process modules, so no code reaches the disk on its own.
