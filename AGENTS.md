...
- **Errors are cause-specific, never generic buckets**: one frozen `EffectonError` dataclass per failure cause, such as `FileNotExists` or `HttpStatusError`. Signatures carry precise unions through `type` aliases. `attempt` error mappers translate only the expected exception types and re-raise the rest so they stay defects. Errors are leaves: mark every error dataclass `@final` and never subclass one, so `catch(T)`'s runtime `isinstance` check and the static subtraction of `T` from `E` always agree. Catch by class, never by predicate. `packages/changesets/src/changesets/shared/file_system.py` is the exemplar.
...
- **Service pattern**: one module per service, exporting `Protocol`, `Live`, and `Test`. Consumers alias the module: `from ... import file_system as FileSystem`, then `FileSystem.Protocol`.
